...
description: 'Real-world patterns for using PinchTab effectively'
---
...
# Create headed instance for debugging
...
# Navigate somewhere
response = requests.post(
    f"http://localhost:9867/instances/{instance_id}/tabs/open",
...
# Cleanup
requests.post(f'http://localhost:9867/instances/{instance_id}/stop')
```
</CodeGroup>
...
async function runHeadlessTests() {
  ...
  console.log(`Created ${instances.length} headless workers`);
  ...
  await Promise.all(
    instances.map(async (instanceId, index) => {
      await fetch(
        `http://localhost:9867/instances/${instanceId}/tabs/open`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ url: testUrls[index] })
        }
      );
      console.log(`Test ${index} completed`);
    })
  );
  ...
  await Promise.all(
    instances.map(id => 
      fetch(`http://localhost:9867/instances/${id}/stop`, { method: 'POST' })
    )
  );
}
```
...
| **Batch parallel** | High-throughput | High | Medium |
