...
description: Connect PinchTab to remote Chrome instances using the Chrome DevTools Protocol
icon: link
---
...
<Steps>
  <Step title="Start shared Chrome once">
    ```bash
...
<Accordion title="Connection refused or Failed to open new tab">
...
<Accordion title="Port and networking issues">
**Local machine only:**
```bash
...
<CodeGroup>
```yaml Option 1: Internal Network Only
...
```yaml Option 2: Internal Service Only
...
```bash Option 3: SSH Tunnel for External Access
